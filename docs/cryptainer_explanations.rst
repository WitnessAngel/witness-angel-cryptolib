...
Unique IDs of a cryptainer
----------------------------

The `cryptainer_uid` field, located at the root of a cryptainer, uniquely identifies.

The `keychain_uid` value located near, on the contrary, can be shared by several cryptainers, which thus end up using a common keychain of keypairs generated by trustees (keypairs being differentiated by their key-algo value).
However this default `keychain_uid` can also be overridden deeper in the conf tree, at nodes specifying trustees.
