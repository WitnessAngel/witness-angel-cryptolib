...
def profile_simple_encryption():
    ...
    cryptainer_storage = CryptainerStorage(
        default_cryptoconf=CRYPTOCONF, cryptainer_dir=tmp_path, offload_payload_ciphertext=True
    )
    ...
    tarfile_aggregator.add_record(
        sensor_name="dummy_sensor", from_datetime=now, to_datetime=now, extension=".bin", payload=payload
    )
