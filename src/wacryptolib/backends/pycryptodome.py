...
def export_rsa_key_to_pem(private_key, passphrase=None):
    extra_params = dict(passphrase=passphrase, pkcs=8, protection="PBKDF2WithHMAC-SHA1AndAES256-CBC") if passphrase else {}  # FIXME
    return private_key.export_key(format="PEM", **extra_params)


def export_dsa_key_to_pem(private_key, passphrase=None):
    extra_params = dict(passphrase=passphrase, pkcs8=True, protection="PBKDF2WithHMAC-SHA1AndDES-EDE3-CBC") if passphrase else {}  # FIXME
    return private_key.export_key(format="PEM", **extra_params)


def export_ecc_key_to_pem(private_key, passphrase=None):
    extra_params = dict(passphrase=passphrase, use_pkcs8=True, protection="PBKDF2WithHMAC-SHA1AndAES128-CBC") if passphrase else {}  # FIXME
    return private_key.export_key(format="PEM", **extra_params)
