...
class ContainerWriter(ContainerBase):
    ...
    def _encrypt_symmetric_key(self, container_uid: uuid.UUID, symmetric_key_data: bytes, conf: dict) -> bytes:
        ...
        subkey = KEY_TYPES_REGISTRY[subkey_type]["pem_import_function"](subkey_pem)  # FIXME
...
class ContainerReader(ContainerBase):
    ...
    def decrypt_data(self, container: dict) -> bytes:
        ...
        for data_encryption_stratum in reversed(container["data_encryption_strata"]):
            ...
            for key_encryption_stratum in data_encryption_stratum["key_encryption_strata"]:
                symmetric_key_cipherdict = load_from_json_bytes(symmetric_key_data)  # Remain as bytes all along
    ...
    def _decrypt_symmetric_key(self, container_uid: uuid.UUID, symmetric_key_cipherdict: dict, conf: list):
    ...
    def _verify_signatures(self, container_uid: uuid.UUID, message: bytes, conf: dict):
        subkey_type, signature_type = conf["signature_type"]
        encryption_proxy = _get_proxy_for_escrow(conf["signature_escrow"])
        public_key_pem = encryption_proxy.get_public_key(uid=container_uid, key_type=subkey_type)
        public_key = KEY_TYPES_REGISTRY[subkey_type]["pem_import_function"](public_key_pem)

        print("\n> VERYFYING SIGNATURE FOR \n%s with %s key of public form %s" % (message, public_key.__class__, public_key.export_key(format="PEM")))
