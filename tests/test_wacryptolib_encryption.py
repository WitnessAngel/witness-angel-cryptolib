...
def test_aes_cbc_encryption_and_decryption():
    ...
    iv_and_ciphertext = wacryptolib.encryption.encrypt_via_aes_cbc(
        key=key, plaintext=binary_content
    )

    decrypted_content = wacryptolib.encryption.decrypt_via_aes_cbc(
...
def test_aes_eax_encryption_and_decryption():
    ...
    encryption = wacryptolib.encryption.encrypt_via_aes_eax(
        key=key, plaintext=binary_content
    )

    decrypted_content = wacryptolib.encryption.decrypt_via_aes_eax(
...
def test_chacha20_symetric_encryption_and_decryption():
    ...
    encryption = wacryptolib.encryption.encrypt_via_chacha20_poly1305(
        key, binary_content, header=b"header"
    )

    decrypted_content = wacryptolib.encryption.decrypt_via_chacha20_poly1305(
        key=key, encryption=encryption
    )
...
def test_rsa_oaep_encryption_and_decryption():
    ...
    encryption = wacryptolib.encryption.encrypt_via_rsa_oaep(
        key=keypair["public_key"], plaintext=binary_content
    )

    decrypted_content = wacryptolib.encryption.decrypt_via_rsa_oaep(
